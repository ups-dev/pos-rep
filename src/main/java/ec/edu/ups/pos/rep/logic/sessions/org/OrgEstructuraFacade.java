...
public class OrgEstructuraFacade extends AbstractFacade<OrgEstructura> {
    ...
    public List<OrgEstructura> listaEstructuraSedeActivo() {
        Query q = getEntityManager().createQuery("SELECT t FROM OrgEstructura t WHERE t.orgDescripcionEstructura.tieCodigo = 2L AND t.audEliminado = 'N' ORDER BY t.estCodigo ");
    ...
    public List<OrgEstructura> listaEstructuraCampusActivo(OrgEstructura orgEstructuraSede) {
        Query q = getEntityManager().createQuery("SELECT t FROM OrgEstructura t WHERE t.orgEstructuraPadre= :orgEstructuraSede AND t.orgDescripcionEstructura.tieCodigo IN (3L,4L) AND t.audEliminado = 'N' ORDER BY t.estCodigo ")
                .setParameter("orgEstructuraSede", orgEstructuraSede);
        
        return q.getResultList();
    }
    
    //Obtiene la lista de OrgEstructuraCarrera seleccionado un campus y todos las carreras
    public List<OrgEstructura> listaEstructuraCarreraSelecActivo(OrgEstructura orgEstructuraCampus) {
        Query q = getEntityManager().createQuery("SELECT t FROM OrgEstructura t WHERE t.orgEstructuraPadre= :orgEstructuraCampus AND t.orgDescripcionEstructura.orgTipoEstructura.tieCodigo = 5L AND t.audEliminado = 'N' ORDER BY t.orgDescripcionEstructura.deeDescripcion ASC ")
                .setParameter("orgEstructuraCampus", orgEstructuraCampus);
        
        return q.getResultList();
    }
    
    //Obtiene la lista de OrgEstructuraPostGrado seleccionado un campus y todos las carreras
    public List<OrgEstructura> listaEstructuraPostGradoSelecActivo(OrgEstructura orgEstructuraCampus) {
        Query q = getEntityManager().createQuery("SELECT t FROM OrgEstructura t WHERE t.orgEstructuraPadre= :orgEstructuraCampus AND t.orgDescripcionEstructura.orgTipoEstructura.tieCodigo = 6L AND t.audEliminado = 'N' ORDER BY t.orgDescripcionEstructura.deeDescripcion ASC")
                .setParameter("orgEstructuraCampus", orgEstructuraCampus);
