...
@Named(value = "insAlumnoController")
@ViewScoped
public class InsAlumnoController extends AbstractController<InsAlumno> {
    ...
    public InsAlumno prepareCreate(ActionEvent event) {
        clienteLocalToEdit = new ClienteLocal();        
     ...
     */
    public void searchByCedulaFull(Integer origen) { //Se usa para buscar el alumno en alumno       
        clienteLocalToEdit = clienteLocalFacade.findClienteLocalByCedula(cedula);
        InsAlumno insAlumno = super.prepareCreate(null);
        if (clienteLocalToEdit != null) {
            InsAlumno insAlumnoTemp = insAlumnoFacade.findByCllcCdg(clienteLocalToEdit.getCllcCdg());
            if (insAlumnoTemp != null) {
                insAlumno = insAlumnoTemp;
            } else {
                insAlumno = setDataAlumno(insAlumno, clienteLocalToEdit);
                insAlumno.setCllcCdg(clienteLocalToEdit.getCllcCdg());
            }            
        }         
    ...
    public void setSelectedToEdit(InsAlumno selectedToEdit) {
        if (selectedToEdit != null) {            
            if (selectedToEdit.getCllcCdg() != null) {
                ClienteLocal clienteLocal = clienteLocalFacade.find(selectedToEdit.getCllcCdg());
                if (clienteLocal.getCllcRuc() != null) {
                    cedula = clienteLocal.getCllcRuc();                    
                }
            }
        }
    ...
    public ClienteLocal buscarClienteLocal(Object cllcCdg) {
        ...
        return clienteLocalFacade.find(cllcCdg);
    }
